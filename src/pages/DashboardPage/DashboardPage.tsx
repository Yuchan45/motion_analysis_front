...
export function DashboardPage() {
  ...
  async function load() {
    ...
    try {
      ...
      setError(reason instanceof Error ? reason.message : "No se pudo cargar la biblioteca.");
    } finally { setLoading(false); }
  }

  useEffect(() => { void load(); }, []);

  async function remove(video: VideoResource) {
    if (!window.confirm(`¿Eliminar ${video.title} y todos sus análisis?`)) return;
    await apiRequest<void>(`/videos/${video.id}`, { method: "DELETE" });
    setItems((current) => current.filter((item) => item.id !== video.id));
  }

  async function rename(video: VideoResource) {
    const title = window.prompt("Nuevo título", video.title)?.trim();
    if (!title || title === video.title) return;
    await apiRequest(`/videos/${video.id}`, { method: "PATCH", body: JSON.stringify({ title }) });
    await load();
  }

  return <main className="library-shell">
    <section className="library-hero">
      <div><p className="eyebrow">Tu biblioteca</p><h1>Videos y análisis, siempre disponibles.</h1></div>
      <Link className="primary-link" to="/analysis/new">+ Subir video</Link>
    </section>
